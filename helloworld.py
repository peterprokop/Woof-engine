...
class FeedImage(db.Model):
    ...
    def set_image(self, image_data):
        ...
        resized_image_data = image.execute_transforms(output_encoding=images.JPEG)
        self.data = db.Blob(resized_image_data)
...
class Guestbook(webapp.RequestHandler):
    def post(self):
        ...
        if users.get_current_user():
            feed_item.author = users.get_current_user()
        feed_item.text = self.request.get("text")
        #image = images.resize(self.request.get("img"), width = 320, output_encoding = images.JPEG)
        #feed_item.avatar = db.Blob(avatar)
        feed_item.image = FeedImage().set_image(self.request.get("img")).key()
        feed_item.put()
        self.redirect('/')
